...
class GPSSimulator:
    ...
    def __init__(self):
        ...
        self.pub = rospy.Publisher("~odometry_out", Odometry, queue_size=10)
            ...
            #self.diff_pub = rospy.Publisher("~difference", OdometryOffset, queue_size=10)
        if self.publish_tf:
    ...
    def odom_callback(self, ros_data):
        ...
        # Publish difference
        if self.publish_difference:
            new_ori = np.array([ros_data.pose.pose.orientation.x, ros_data.pose.pose.orientation.y,
                                ros_data.pose.pose.orientation.z, ros_data.pose.pose.orientation.w])
            new_pos = np.array([ros_data.pose.pose.position.x, ros_data.pose.pose.position.y,
                                ros_data.pose.pose.position.z])
            """msg = OdometryOffset()
        ...
        # tf
        if self.publish_tf:
            self.tf_br.sendTransform((ros_data.pose.pose.position.x, ros_data.pose.pose.position.y,
                                      ros_data.pose.pose.position.z),
    ...
    def apply_noise_random_walk(self, ros_data):
        ...
        while count < self.max_tries:
            distance = random.uniform(self.walk_speed_range[0], self.walk_speed_range[1]) * time_diff * \
                       self.position_uncertainty
            theta = 2.0 * math.pi * random.random()
            phi = math.acos(1 - 2.0 * random.random())  # to make points uniformly distributed on sphere
            direction = np.array([math.sin(phi) * math.cos(theta), math.sin(phi) * math.sin(theta), math.cos(phi)])
            if np.linalg.norm(self.current_offset[:3] + direction * distance) <= self.position_uncertainty:
                self.current_offset[:3] = self.current_offset[:3] + direction * distance
                break
            else:
                count += 1
        ...
        while count < self.max_tries:
            ...
            if math.fabs(self.current_offset[5] + offset) < self.yaw_uncertainty:
        ...
        orientation = tf.transformations.quaternion_from_euler(r, p, y)
        ros_data.pose.pose.position.x = ros_data.pose.pose.position.x + self.current_offset[0]
        ros_data.pose.pose.position.y = ros_data.pose.pose.position.y + self.current_offset[1]
        ros_data.pose.pose.position.z = ros_data.pose.pose.position.z + self.current_offset[2]
        ros_data.pose.pose.orientation.x = orientation[0]
        ros_data.pose.pose.orientation.y = orientation[1]
        ros_data.pose.pose.orientation.z = orientation[2]
        ros_data.pose.pose.orientation.w = orientation[3]
