...
# *** Args (need to be set) ***
n_experiments=12
target_dir="/media/davide/DATA/data/submap_exploration_planner/WithDrift/Drift0_001"		# Can reuse same dir to add experiments
...
do  
  ...
  # evaluate
   roslaunch active_3d_planning_app_submap_exploration eval_data.launch target_directory:=$target_dir method:=recent series:=false clear_voxblox_maps:=$clear_voxgraph_maps evaluate:=true experiment_config:="Maze.yaml"
done
